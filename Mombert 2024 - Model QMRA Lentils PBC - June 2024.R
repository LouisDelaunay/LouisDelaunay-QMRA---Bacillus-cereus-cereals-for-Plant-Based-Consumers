...
muopt_Bc3_L <- 2.05 # µopt for tubers and lentils - Combase ID: Car_45 ; pH 6.5 ; temperature 30; G= 0.346 µ = 2.05
z_Bc3       <- 6.2
D90_Bc3     <- 101.9
...
muopt_Bc4_L <- 2.05 # µopt for tubers and lentils - Combase ID: Car_45 ; pH 6.5 ; temperature 30; G= 0.346 µ = 2.05
z_Bc4       <- 7.128
D100_Bc4    <- 6.08
...
#------------------------------- Results and heat inactivation treatment
...
pct_Bc_inf_2log <- (N_inf_2log/(N_inf_2log + N_sup_2log))
P_Bc_inf_2log   <- rep(pct_Bc_inf_2log, 100)

  # Dishes contaminated with 2 to 3 log CFU.g-1
N0_Bc_sup_2log  <- rpert(100, min=100*dish_size, mode=1000*dish_size ,max=Cmax*dish_size) #quantity of B. cereus per dish
pct_Bc_sup_2log <- (N_sup_2log/(N_inf_2log + N_sup_2log))
...
p_surv_Bc3_L  <- 10^(-tht_P/Dht_Bc3) #survival probability of bacterial spores from group III B. cereus

# Survival of bacterial spores from group IV B. cereus after heat treatment
p_surv_Bc4_L  <- 10^(-tht_P/Dht_Bc4) #survival probability of bacterial spores from group IV B. cereus
...
#---- Calculating risks
risk_hot_L          <- C_ht_Bc_L > cutoff 

risk_24h_L          <- C_24h_Bc_L > cutoff 
risk_48h_L          <- C_48h_Bc_L > cutoff 
risk_72h_L          <- C_72h_Bc_L	> cutoff 
risk_96h_L          <- C_96h_Bc_L > cutoff 
risk_120h_L         <- C_120h_Bc_L > cutoff

risk_24h_nofridge_L <- C_24h_nofridge_Bc_L > cutoff
